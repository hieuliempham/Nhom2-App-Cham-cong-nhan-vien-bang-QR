...
public class AttendaceActivity extends AppCompatActivity {
    ...
    private void copyText(String text) {
        ...
        Toast.makeText(AttendaceActivity.this, "Lưu vô clipboard", Toast.LENGTH_LONG).show();
    }
    ...
    private void exportExcel() {
        ...
        */
        ...
        for (int i = 0; i < arrayList.get(0).size(); i++) {
            ...
            // Thêm cột Lương (200k mỗi lần quét)
            temp.add("200000");
        ...
        int totalSalary = 0;
        for (ArrayList<String> temp : reformed) {
            int salary = Integer.parseInt(temp.get(3)); // Lấy giá trị từ cột Lương
            totalSalary += salary;
        }

        // Thêm cột Tổng vào danh sách
        for (ArrayList<String> temp : reformed) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.Q) {
            if (createXLS(reformed)) {
                makeToast("Đã file XLS xuất thành công.");
            } else {
                makeToast("Không thể xuất XLS file.");
            }
        }
    }
    ...
    public boolean createXLS(ArrayList<ArrayList<String>> list) {
        ...
        cell.setCellValue(CONCAT_COL_HEADER); // Tên cột Lương
        cell.setCellStyle(style);
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            cell = row.createCell(3);
            cell.setCellValue(temp.get(3)); // Cột Lương
            cell.setCellStyle(concatStyle);

            // Thêm cột Tổng
            cell = row.createCell(4);
            cell.setCellValue(temp.get(4)); // Cột Tổng
            cell.setCellStyle(concatStyle);
        }
